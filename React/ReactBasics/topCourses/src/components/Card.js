...
function Card(props) {
    ...
    let setLikeedCourses = props.setLikeedCourses;
    ...
    function clickHandler() {
        ...
        if (likedCourses.includes(course.id)) {
            ...
            setLikeedCourses((prev) => prev.filter((cid) => (cid != course.id)));
            ...
            if (likedCourses.length === 0) {
                setLikeedCourses([course.id])
            }
            else {
                //non empty pehle se kuch hai
                setLikeedCourses((prev) => [...prev, course.id]);
      ...
      <div className="w-[300px] bg-slate-600 rounded-md overflow-hidden">
        <div className="relative ">
          <img src={course.image.url} />
           ...
           bottom-1 grid place-items-center">
